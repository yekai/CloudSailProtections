...
- (void)updateBar
{
    ...
    for (NSInteger i = 0; i < _yValues.count; i++) {
        ...
        if (_bars.count == _yValues.count) {
            ...
            bar.labelTextFontSize = _labelTextFontSize;
...
//        float grade =fabsf((float)value / (float)_yValueMax);
        float grade = fabsf(value);
        if (isnan(grade)) {
            grade = 0;
        }
        bar.maxDivisor = (float)_yValueMax;
        bar.grade = grade;
        bar.displayText = _displayTexts[i];
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([subview isKindOfClass:[PNBar class]] && [self.delegate respondsToSelector:@selector(userClickedOnBarAtIndex:lineChart:)]) {
