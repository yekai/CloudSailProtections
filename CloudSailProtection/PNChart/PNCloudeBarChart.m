...
#import "PNCloudeBarChart.h"
#import "PNColor.h"
#import "PNChartLabel.h"

@interface PNCloudeBarChart () {
...
@implementation PNCloudeBarChart
...
- (void)setupDefaultValues
{
    ...
    _yLabelPrefix        = @"";
    _yLabelSuffix        = @"";
    _xyLabelTextColor    = [UIColor blackColor];
...
- (void)__addYCoordinateLabelsValues{
  ...
  for (int i = 0; i <= _yLabelSum; i++) {
    ...
    label.textColor = _xyLabelTextColor;
        ...
        for (int index = 0; index < _xLabels.count; index++) {
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                label.center = CGPointMake(labelXPosition,
                                           self.frame.size.height - kXLabelHeight - _chartMarginTop + label.frame.size.height  + _labelMarginTop);
                labelAddCount = 0;
...
- (void)updateBar
{
    ...
    for (NSNumber *valueString in _yValues) {
        ...
        if (_bars.count == _yValues.count) {
            ...
            bar = [[PNCloudBar alloc] initWithFrame:CGRectMake(barXPosition, //Bar X position
